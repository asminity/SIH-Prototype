...
export function InvestigationEvidence({ factors, clusterTransactionCount, walletCount, ipCount }: InvestigationEvidenceProps) {
  const cards = [
    { label: 'Evidence 01', title: factors[1]?.label ?? 'Repeated IP-Wallet Association', confidence: '91%', observed: `${ipCount} associated addresses`, summary: factors[1]?.evidence ?? 'Repeated network and wallet observations were retained as correlation evidence.', path: factors[1]?.evidencePath ?? '/network-activity' },
